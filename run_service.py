...
from operate.operate_types import (
    ...
    FundRequirementsTemplate, OnChainState,
)
...
@dataclass
class MindshareConfig(LocalResource):
    """Local configuration."""
...
def load_local_config() -> MindshareConfig:
    """Load the local mindshare configuration."""
    path = OPERATE_HOME / "local_config.json"
    if path.exists():
        mindshare_config = MindshareConfig.load(path)
    else:
        mindshare_config = MindshareConfig(path)
    
    return mindshare_config

def configure_local_config() -> MindshareConfig:
    """Configure local mindshare configuration."""
    path = OPERATE_HOME / "local_config.json"
    if path.exists():
        mindshare_config = MindshareConfig.load(path)
    else:
    ...
    if mindshare_config.tenderly_access_key is None:
        print_section("Tenderly API Configuration and Price Data Source")
        mindshare_config.tenderly_access_key = get_masked_input(
            "Please enter your Tenderly API Key. Get one at https://dashboard.tenderly.co/: "
        )

    if mindshare_config.tenderly_account_slug is None:
        mindshare_config.tenderly_account_slug = get_masked_input(
            "Please enter your Tenderly Account Slug: "
        )

    if mindshare_config.tenderly_project_slug is None:
        mindshare_config.tenderly_project_slug = get_masked_input(
            "Please enter your Tenderly Project Slug: "
        )

    if mindshare_config.coingecko_api_key is None:
        mindshare_config.coingecko_api_key = get_masked_input(
            "Please enter your CoinGecko API Key. Get one at https://www.coingecko.com/: "
        )
        print()

    if mindshare_config.min_swap_amount_threshold is None:
        ...
        if update_min_swap:
            while True:
                ...
                if int(min_swap_amount) >= DEFAULT_MIN_SWAP_AMOUNT_THRESHOLD:
                    mindshare_config.min_swap_amount_threshold = min_swap_amount
                    break
                else:
                    print(f"Error: The minimum investment amount must be at least {DEFAULT_MIN_SWAP_AMOUNT_THRESHOLD} USD.")
        else:
            mindshare_config.min_swap_amount_threshold = str(DEFAULT_MIN_SWAP_AMOUNT_THRESHOLD)
        print()

    if mindshare_config.password_migrated is None:
        mindshare_config.password_migrated = False

    if mindshare_config.use_staking is None:
        print_section("Staking") 
        mindshare_config.use_staking = input("Do you want to stake your service? (y/n): ").lower() == 'y'

    if mindshare_config.staking_chain is None:
        if mindshare_config.use_staking:
            print("Possible staking chains are Base, with Base being the default setting.")
            switch_staking = input("Do you want to switch such setting to Base (y/n)?: ").lower() == 'y'
            if switch_staking:
                mindshare_config.staking_chain = "base"
                print("The staking chain has changed to Base.")
            else:
                mindshare_config.staking_chain = "base"

    if mindshare_config.principal_chain is None:
        mindshare_config.principal_chain = mindshare_config.staking_chain if mindshare_config.staking_chain else DEFAULT_STAKING_CHAIN.lower()

    if mindshare_config.investment_funding_requirements is None:
        mindshare_config.investment_funding_requirements = {
            mindshare_config.principal_chain : {
                "USDC": 0,
                "ETH": 0
            }
        }
    ...
    print("All available chains for trading opportunities:", ", ".join(DEFAULT_CHAINS))
    print("Current setting for trading opportunities--Chains:", ", ".join(mindshare_config.target_investment_chains or [DEFAULT_STAKING_CHAIN]))
    ...
    if update_chains:
        ...
        if mindshare_config.principal_chain not in allowed_chains:
            allowed_chains.append(mindshare_config.principal_chain)

        mindshare_config.allowed_chains = allowed_chains
        mindshare_config.target_investment_chains = target_investment_chains
    
    if mindshare_config.allowed_chains is None:
        mindshare_config.allowed_chains = [mindshare_config.principal_chain]
    
    if mindshare_config.target_investment_chains is None:
        mindshare_config.target_investment_chains = [mindshare_config.principal_chain]

    for chain in mindshare_config.allowed_chains:
        if chain == "base" and mindshare_config.base_rpc is None:
            mindshare_config.base_rpc = get_masked_input("Please enter a Base RPC URL: ")
        print()

    if mindshare_config.selected_strategies is None:
        mindshare_config.selected_strategies = [Strategy.BalancerPoolSearchStrategy.value, Strategy.UniswapPoolSearchStrategy.value]

    mindshare_config.store()
    return mindshare_config
...
def handle_password_migration(operate: OperateApp, config: MindshareConfig) -> t.Optional[str]:
...
def get_service_template(config: MindshareConfig) -> ServiceTemplate:
    """Get the service template"""
    home_chain_id = "8453"
    return ServiceTemplate({
        "name": "Mindshare",
        "hash": "bafybeigrzisajgvi2qavzrda6wtwol7irjajgf7rjyll5fg7filxj3rvgy",

        "description": "Mindshare",
        "image": "https://gateway.autonolas.tech/ipfs/bafybeic73qt7npiu55w76q4ipzvqkzfk5rr6sacyqe6crpyip7qddaajtq",
        "service_version": 'v0.1.0',
        "home_chain_id": home_chain_id,
        "configurations": {
            "8453": ConfigurationTemplate(
                {
                    "staking_program_id": "0x8b8a86A056D3145Ec2ecc30FB52C3aF2e6daB7C5",
                    "rpc": config.base_rpc,
...
FALLBACK_STAKING_PARAMS = dict(
    agent_ids=[73],
    service_registry="0x3C1fF68f5aa342D296d4DEe4Bb1cACCA912D95fE",  # nosec
    ...
    activity_checker="0x133FcEF52285f9A9530A18C6E82c5dd931E175F5"  # nosec
)
