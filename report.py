...
def get_chain_rpc(mindshare_config, chain_name):
    chain_name_to_rpc = {
        'base': mindshare_config.base_rpc,
    }
...
def generate_report():
    try:
        ...
        print("")
        print("==============")
        print("Mindshare Service Report")
        print("Generated on: ", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        print("==============")

        staking_report(config)
        home_chain = config.get("home_chain")
        service_id = config.get("chain_configs", {}).get(str(home_chain), {}).get("chain_data", {}).get("token")
        if not service_id:
        ...
        for chain_id, chain_config in config.get("chain_configs", {}).items():
            ...
            # Low balance check
            agent_threshold_wei = fetch_agent_fund_requirement(chain_id, get_chain_rpc(mindshare_config, chain_name))
        ...
        for chain_id, chain_config in config.get("chain_configs", {}).items():
            ...
            if  mindshare_config.allowed_chains and chain_name.lower() not in mindshare_config.allowed_chains:
                continue
            if mindshare_config.target_investment_chains and chain_name.lower() not in mindshare_config.target_investment_chains:
