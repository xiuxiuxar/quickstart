...
def get_chain_name(chain_id, chain_id_to_metadata):
    # For now, we only support Base chain
    if str(chain_id).lower() == 'base':
        return "Base"
    
    # Fallback for numeric chain IDs
    try:
...
def validate_config(config):
    required_keys = ['home_chain', 'chain_configs']
...
def _get_agent_status() -> str:
    # Find the most recently modified service directory with a docker-compose file
    services_dir = OPERATE_HOME / "services"

    if not services_dir.exists():
        return _color_string("No Service", ColorCode.RED)

    service_dirs = [d for d in services_dir.iterdir() if d.is_dir() and not d.name.startswith("invalid_")]
    if not service_dirs:
        return _color_string("No Service", ColorCode.RED)

    # Find the service with a docker-compose file
    docker_compose_path = None
    for service_dir in sorted(service_dirs, key=lambda x: x.stat().st_mtime, reverse=True):
        potential_compose = service_dir / "deployment" / "docker-compose.yaml"
        if potential_compose.exists():
            docker_compose_path = potential_compose
            break

    if not docker_compose_path:
        return _color_string("Not Deployed", ColorCode.YELLOW)
    try:
