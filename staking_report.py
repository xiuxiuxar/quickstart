...
def staking_report(config: dict) -> None:
    try:
        ...
        home_chain = config.get("home_chain")
        if not home_chain:
            print("Error: 'home_chain' not found in config.")
            return

        # Get chain configuration - home_chain might be a name like "base" or an ID
        chain_config_key = str(home_chain)
        chain_config = config.get("chain_configs", {}).get(chain_config_key)

        if not chain_config:
            print(f"Error: Chain configuration not found for {home_chain}.")
            return

        service_id = chain_config.get("chain_data", {}).get("token")
        if not service_id:
            print(f"Error: 'token' not found in chain data for {home_chain}.")
            return

        multisig_address = chain_config.get("chain_data", {}).get("multisig")
        if not multisig_address:
            print(f"Error: 'multisig' address not found in chain data for {home_chain}.")
            return

        w3 = Web3(HTTPProvider(rpc))

        # Map chain name to ChainType and chain ID
        chain_name_mapping = {
            "base": (ChainType.BASE, 8453),
            "ethereum": (ChainType.ETHEREUM, 1),
            "gnosis": (ChainType.GNOSIS, 100),
            "optimism": (ChainType.OPTIMISM, 10),
            "mode": (ChainType.MODE, 34443),
        }

        home_chain_lower = home_chain.lower()
        if home_chain_lower in chain_name_mapping:
            home_chain_type, home_chain_id = chain_name_mapping[home_chain_lower]
        else:
            print(f"Error: Unsupported chain {home_chain}.")
            return
        # staking_token_address = STAKING.get(home_chain_type, {}).get("optimus_alpha")
        staking_token_address = "0x8b8a86A056D3145Ec2ecc30FB52C3aF2e6daB7C5"
